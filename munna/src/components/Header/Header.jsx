...
const Header = () => {
  ...
  const [searchActive, setSearchActive] = useState(false); // State to toggle search bar
            ...
            <div className="col-12">
              <div className="header-contain">
                ...
                <div style={{ position: 'relative' }}>
                  <div
                    ref={searchRef}
                    className={`header-search ajax-search the-basics dflex responsive-search ${searchActive ? 'active' : ''}`}
                    style={{
                      backgroundColor: "white",
                      border: "1px solid rgb(71, 4, 242)",
                      padding: "0px 3px",
                    }}
                  >
                    <div
                    ...
                    <span className="close-searchbar" onClick={() => setSearchActive(false)}>
                      <i className="fa fa-times" style={{ color: "purple" }} />
                    </span>
                  </div>
                  <ul className="serachlisting" style={{ display: 'flex', position: 'absolute', width: '100%', flexDirection: 'column', zIndex: 4, background: "#fff" }}>
                    {isError === false ? searchapidata?.results?.[0]?.product_name ? searchapidata?.results.map((item, index) => (
                      <li key={index}>
                        <div className="p-1 d-flex" style={{ gap: "10px", cursor: 'pointer' }} onClick={() => { transfer(item._id) }}>
                          <div className="imagecontain" style={{ width: '47%' }}>
                            <img src={`${process.env.REACT_APP_API_IMAGE_URL}${item.product_image1}`} style={{ width: '100%', height: '100%' }} className="img-fluid" alt="product" />
                          </div>
                          <div className="cartinfo">
                            <h6 style={{ fontSize: '12px', color: 'purple', fontWeight: '600', padding: '3px 0px' }}>{item.product_name}</h6>
                            <h6 style={{ fontSize: '12px', color: 'purple', padding: '3px 0px' }}>₹{item.selling_price}</h6>
                            <p style={{
                              fontSize: '11px', color: 'purple', display: "-webkit-box",
                              WebkitLineClamp: "2",
                ...
                <div className="icon-block">
                  <ul className="theme-color">
                    ...
                    <li className="search-icon" onClick={() => setSearchActive(!searchActive)}>
                      <i className="fa fa-search" style={{ color: "purple", fontSize: "20px" }} />
                    </li>
                  </ul>
                </div>
              </div>
