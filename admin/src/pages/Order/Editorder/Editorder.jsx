...
import img from "../../../assets/jubair.png";
...
const Editorder = () => {
  ...
  if (orderloading || websiteinfoloading) {
    return <></>;
  }
        ...
        <section className="invoice">
                          ...
                          <tbody>
                            <tr>
                              <td width="49%"><strong>Company Name :</strong>{websiteinfo?.data?.[0]?.website_name}<br />
                                <strong>Address:</strong>  P.No-G-155. 2nd Floor. Sector D.|P-3, Tronica City, Ghaziabad (UP) 201103. 
                                Mob N0. 7861020500 <br />
                                <strong>Phone no.: </strong>+91{websiteinfo?.data?.[0]?.mobile_no}<br />
                                <strong>Email: </strong>{websiteinfo?.data?.[0]?.email}<br />
                                <strong>GSTIN:</strong>  09JPYS0803H1Z6 
                              </td>
          ...
          <div className="row removeprintbtn" style={{ padding: '10px 22px', display: 'flex', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', width: '60%', padding: '0px' }} className='hidebox1'>
              ...
              <button className="save-button hidebox1" type='button' onClick={() => { changestatus(status, orderdetail?.data?._id) }} style={{ background: 'purple' }} >
                Save
              </button>
            </div>
            <div style={{ display: 'flex', justifyContent: 'end', width: '40%' }} className='hidebox1'>
              <button onClick={handlePrint} className="removeprintbtn printbtn className='hidebox1'" style={{ background: "purple" }}>
